  ...
  return (
    <main>
      ...
      <article className="article container">
        ...
        <section><p className="eyebrow">DAY BY DAY</p><h2>逐日行程</h2>{guide.itinerary.map((day) => <div className="day" key={day.day}><div className="day-number">{String(day.day).padStart(2, "0")}</div><div><h3>{day.title}</h3><ol>{day.items.map((item, index) => <li key={`${item.place}-${index}`}><strong>{item.time && `${item.time} · `}{item.place}</strong><p>{item.description}</p>{item.tips && <p className="tip">提示：{item.tips}</p>}</li>)}</ol></div></div>)}</section>
