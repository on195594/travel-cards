...
export function GuideEditor({ initialGuide }: { initialGuide?: Guide }) {
  ...
  async function uploadCover(file: File | undefined) {
    ...
    try {
      ...
      if (!response.ok) { setMessage(data?.error?.message ?? "图片上传失败，请重试。"); return; }
      field("coverImage", { objectKey: data.objectKey, publicUrl: data.publicUrl, alt: draft.coverImage?.alt ?? "" });
  ...
  return (
    <>
      <header className="admin-header"><div><Link href="/admin/guides">← 攻略列表</Link><h1>{saved ? "编辑攻略" : "新建攻略"}</h1>{saved && <p><span className={`status ${saved.status}`}>{saved.status === "published" ? "已发布" : "草稿"}</span> · 版本 {saved.revision}</p>}</div><div className="actions"><button type="button" disabled={busy} onClick={save}>{busy ? "处理中…" : "保存攻略"}</button>{saved?.status === "published" ? <button className="secondary" type="button" disabled={busy} onClick={() => transition("unpublish")}>撤回</button> : <button className="secondary" type="button" disabled={busy} onClick={() => transition("publish")}>发布攻略</button>}{saved && <button className="danger" type="button" disabled={busy} onClick={remove}>删除攻略</button>}</div></header>
