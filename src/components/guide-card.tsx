...
export function GuideCard({ guide }: { guide: Guide }) {
  ...
  return (
    <article className="card">
      ...
      <Link className="card-media-link" href={href} aria-label={`查看攻略：${guide.title}`}>{guide.coverImage ? <div className="card-media"><img className="card-image" src={guide.coverImage.publicUrl} alt={guide.coverImage.alt} /><span className="card-arrow" aria-hidden="true">↗</span></div> : <div className="card-media card-media-empty" aria-hidden="true"><span>TRAVEL<br />NOTE</span></div>}</Link>
